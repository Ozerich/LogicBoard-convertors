...
{
    $text = preg_replace("#(\[quote.+?) post=.+?\&\#39;#sui", "\\1", $text);

    $text = preg_replace("#(\[quote.+?).*?name=\&\#39;(.+?)\&\#39;\s#sui", "[quote=\\2 ", $text);


    preg_match_all('#\[quote=.+?\ |timestamp=\&\#39;(.+?)\&\#39;\]#sui', $text, $quotes, PREG_SET_ORDER);
    ...
    $text = preg_replace("#<a href='index.php\?showtopic=(\d+)'>(.+?)</a>#sui", '[url=?do=board&op=topic&id=\\1]\\2[/url]', $text);
