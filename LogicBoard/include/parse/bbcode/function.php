...
function bb_decode($msg)
{
    ...
    $msg = preg_replace("#\[color=[a-zA-Z]+?\](.+?)\[/color\]#sui", "[color=#000000]\\1[/color]", $msg);
    $msg = preg_replace("#\[color=(\#*[0-9ACDEF]+?)\](.+?)\[/color\]#ius", "<font style='color:\\1'>\\2</font>", $msg); //Color
    if (preg_match_all("#\[quote(=((.+?)(\|([0-9\., :]+?))?))?\]#ius", $msg, $shadow) == preg_match_all("#\[/quote\]#ius", $msg, $shadow)) {
        $msg = preg_replace("#\[quote\]#ius", "<blockquote class=\"blockquote\"><p><span class=\"titlequote\">" . "Цитата:" . "</span><span class=\"textquote\">", $msg); //quote
        $msg = preg_replace("#\[quote(=((.+?)(\|([0-9\., :]+?))?))?\]#ius", "<blockquote class=\"blockquote\"><p><span class=\"titlequote\">\\3 (\\5) " . "писал:" . "</span><span class=\"textquote\">", $msg); //quote
        $msg = preg_replace("#\[/quote\]#ius", "</span></p></blockquote><!--quote -->", $msg); //quote
        $msg = preg_replace("#<blockquote class=\"blockquote\"><p><span class=\"titlequote\">(.+?) \(\) (.+?)</span>#ius", "<blockquote class=\"blockquote\"><p><span class=\"titlequote\">\\1 \\2</span>", $msg); //quote
    }
    $msg = preg_replace("#\[youtube=http://(www\.)?youtube.com/v/(.+?)\]#ius", '<object width="480" height="385"><param name="movie" value="http://youtube.com/v/\\2?fs=1"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="http://youtube.com/v/\\2?fs=1" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="480" height="385"></embed></object>', $msg); //youtube
    $msg = preg_replace("#\[youtube=http://(www\.)?youtube.com/watch\?v=(.*?)\]#ius", '<object width="480" height="385"><param name="movie" value="http://youtube.com/v/\\2?fs=1"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="http://youtube.com/v/\\2?fs=1" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="480" height="385"></embed></object>', $msg); //youtube
    $msg = preg_replace("#\[youtube=http://(www\.)?rutube.ru/tracks/([0-9])+.html\?v=(.*?)\]#ius", '<object width="470" height="353"><param name="movie" value="http://video.rutube.ru/\\3"></param><param name="wmode" value="window"></param><param name="allowFullScreen" value="true"></param><embed src="http://video.rutube.ru/\\3" type="application/x-shockwave-flash" wmode="window" width="470" height="353" allowFullScreen="true" ></embed></object>', $msg); //rutube
    $msg = preg_replace("#\[youtube=http://(www\.)?video.rutube.ru/(.*?)\]#ius", '<object width="470" height="353"><param name="movie" value="http://video.rutube.ru/\\2"></param><param name="wmode" value="window"></param><param name="allowFullScreen" value="true"></param><embed src="http://video.rutube.ru/\\2" type="application/x-shockwave-flash" wmode="window" width="470" height="353" allowFullScreen="true" ></embed></object>', $msg); //rutube

    $msg = preg_replace("#(^|\s|>)((http://|https://|ftp://|www\.)\w+[^\s\[\]\<]+)#iu", '\\1[url=\\2]\\2[/url]', $msg);
...
function bb_encode($msg)
{
    ...
    $pattern = array(
        "#<blockquote class=\"blockspoiler\"><span class=\"titlespoiler\"><a href='\#' onclick=\"ShowAndHide\('.+?'\); return false;\">" . $spoiler_title . "</a></span><div id='.+?' style='display:none;' class=\"textspoiler\">#ius",
        "#<blockquote class=\"blockspoiler\"><span class=\"titlespoiler\"><a href='\#' onclick=\"ShowAndHide\('.+?'\); return false;\">(.+?)</a></span><div id='.+?' style='display:none;' class=\"textspoiler\">#ius",
    ...
    $pattern = array(
        "#<blockquote class=\"blockquote\"><p><span class=\"titlequote\">" . $lang_message['quote_title'] . "</span><span class=\"textquote\">#ius",
        "#<blockquote class=\"blockquote\"><p><span class=\"titlequote\">(.+?) ?(\(([0-9\., :]+?)\))? " . $lang_message['quote_title2'] . "</span><span class=\"textquote\">#ius",
        "#<blockquote class=\"blockquote\"><p><span class=\"titlequote\">(.+?)</span><span class=\"textquote\">#ius",
    ...
    $msg = preg_replace("#<a href=['\"](\S.+?)['\"]\s*(target=\"_blank\")?\s*>(.+?)</a>#iuse", "bb_url('\\1', '\\3', false)", $msg); //url
    $msg = preg_replace('#<object width="([0-9]){1,4}" height="([0-9]){1,4}"><param name="movie" value="http://youtube.com/v/(.*?)\?fs=1"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="(.*?)" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="([0-9]){1,4}" height="([0-9]){1,4}"></embed></object>#ius', '[youtube=http://youtube.com/v/\\3]', $msg); //youtube
    $msg = preg_replace('#<object width="([0-9]){1,4}" height="([0-9]){1,4}"><param name="movie" value="http://video.rutube.ru/(.*?)"></param><param name="wmode" value="window"></param><param name="allowFullScreen" value="true"></param><embed src="(.*?)" type="application/x-shockwave-flash" wmode="window" width="([0-9]){1,4}" height="([0-9]){1,4}" allowFullScreen="true" ></embed></object>#ius', '[youtube=http://video.rutube.ru/\\3]', $msg); //youtube

    $msg = preg_replace("#<!-- Small_img:([:a-z_\-/\.0-9]+?)\|(left|right|center)? -->(.+?)<!--/Small_img -->#iuse", "bb_create_img_back('\\1|\\2')", $msg); //php	
    $msg = preg_replace("#<center><img src='(\S+?)'\s*(class='lb_img')?\s*/></center>#ius", "[img]\\1[/img]", $msg); //img center
    $msg = preg_replace("#<img src='(\S+?)' align='(left|right)'\s*(class='lb_img')?\s*/>#ius", "[img=\\2]\\1[/img]", $msg); //img left|right

    $msg = preg_replace("#<center>(.+?)</center>#ius", "[center]\\1[/center]", $msg); //center

    $msg = preg_replace("#<img id='smiles_img' src='{TEMPLATE}/bbcode/smiles/([0-9]{3,3})\.gif' />#ius", "::\\1::", $msg); //smailes
...
function bb_url($link, $text, $encode = true)
{
    ...
    {
        if (preg_match("#away\.php\?s\=[http|www](.+?)#ius", $link)) {
            $link = preg_replace("#((.+?)away\.php\?s\=)#ius", "", $link);
