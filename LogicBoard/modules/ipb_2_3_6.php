...
class IPB_2_3_6 extends EngineBase
{
    ...
    public function convert()
    {
        ...
        $groups_masks = $users_id = $posts_key = $posts_attachment = $users_ban = $users_group = array();
        ...
        foreach ($forums as $forum)
        {
            ...
            $this->destSQL->Query("INSERT INTO forums SET id=%,posi=%,parent_id=%,title=%,alt_name=%,description=%,
                    ...
                    last_post_member=%,last_post_member_id=%,last_title=%,sort_order=%,posts=%,rules=%",
                                  $forum['id'], $forum['position'],$forum['parent_id'],$forum['name'],translit($forum['name']),
        ...
        while (true)
        {
            ...
            foreach ($topics as $topic)
            {
                $this->destSQL->Query("INSERT INTO topics SET id=%,forum_id=%,title=%,description=%,date_open=%,date_last=%,
                                      ...
                                      $topic['tid'],$topic['forum_id'],$topic['title'],$topic['description'],
        ...
        while (true)
        {
            ...
            foreach ($posts as $post)
            {
                ...
                $this->destSQL->Query("INSERT INTO posts SET pid=%,topic_id=%,new_topic=%,text=%,post_date=%,edit_date=%,
                                    ...
                                    edit_reason=%",$post['topic_id'],$post['pid'], $post['new_topic'],$text,
