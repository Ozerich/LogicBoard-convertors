...
class IPB_3_1_4 extends EngineBase
{
    ...
    public function convert()
    {
        ...
        g_search=%,g_prefix_end=%", $lb_group_id, $group['g_title'], $group['prefix'], $group['g_is_supmod'],
                                  $group['g_access_cp'], $group['g_access_offline'], serialize($access), $group['g_mem_info'],
        ...
        while (true)
        {
            ...
            foreach ($users as $user)
            {
                ...
                $group = (isset($groups_key[$user['member_group_id']])) ? $groups_key[$user['member_group_id']] :
                        $user['member_group_id'];
                $this->destSQL->Query("INSERT INTO members SET name=%,email=%,reg_date=%,member_group=%,password=%,ip=%,
                                   personal_title=%,reg_status=1,lastdate=%,b_day=%,b_month=%,banned=%,b_year=%,
                                   count_warning=%,posts_num=%", $user['name'], $user['email'], $user['join'], $group,
                ...
                $users_id[$user['member_id']] = $this->destSQL->InsertedId();
